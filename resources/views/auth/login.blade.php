...
<body class="font-sans antialiased text-slate-900 bg-mint-50 min-h-screen flex items-center justify-center p-4">
    ...
    <div
        class="w-full max-w-6xl bg-white rounded-[2.5rem] shadow-2xl shadow-mint-500/10 overflow-hidden flex flex-col md:flex-row min-h-[700px]">
        ...
        <div class="hidden md:flex w-1/2 bg-slate-900 relative items-center justify-center p-12 overflow-hidden">
            ...
            <div class="relative z-10 text-white">
                <div class="mb-8">
                    <span
                        class="bg-white/20 backdrop-blur-md border border-white/30 px-4 py-2 rounded-full text-sm font-bold uppercase tracking-wider">
                        ✨ #1 Healthcare App
                    </span>
                </div>
                ...
                <div
                    class="mt-8 bg-white/10 backdrop-blur-md p-4 rounded-2xl border border-white/20 flex items-center gap-4 max-w-sm">
                    <div class="bg-white p-2 rounded-full">
                        <svg class="w-6 h-6 text-mint-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-5">
                ...
                <div>
                    <div class="flex justify-between items-center mb-2">
                        ...
                        {{-- Opsi Lupa Password bisa dibiarkan atau dihapus tergantung kebijakan --}}
                        {{--
                ...
                <div class="block">
                    <label for="remember_me" class="inline-flex items-center cursor-pointer group">
                        <input id="remember_me" type="checkbox"
                            class="rounded-lg border-slate-300 text-mint-500 shadow-sm focus:ring-mint-500 w-5 h-5"
                            name="remember">
                        <span
                            class="ms-3 text-sm text-slate-600 font-medium group-hover:text-mint-500 transition-colors">Ingat
                            saya di perangkat ini</span>
                    </label>
                </div>

                <button type="submit"
                    class="w-full bg-slate-900 text-white font-bold py-4 rounded-xl shadow-lg shadow-slate-900/20 hover:bg-slate-800 hover:-translate-y-1 transition-all duration-300">
                    Masuk Sekarang
                </button>

                {{-- PERBAIKAN: Link Register Dihapus & Diganti Info --}}
                <div class="text-center mt-6">
                    <p class="text-slate-400 text-xs font-medium bg-slate-50 py-3 rounded-xl">
                        🔒 Belum memiliki akun? <br>
                        Silakan hubungi <span class="text-slate-700 font-bold">IT Support / Superadmin</span>.
                    </p>
                </div>
            </form>
        </div>
